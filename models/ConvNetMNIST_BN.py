...
class ConvNetMNIST_BN(nn.Module):
	def __init__(self,input_shape,num_feature,num_class,transfered=None):
		super(ConvNetMNIST_BN,self).__init__()

		self.conv = nn.Sequential( #input_shape[0]*input_shape[1]*input_shape[2]
			Conv2dSame_ReLU_BN(in_channels=input_shape[0],out_channels=32,kernel_size=3),
			Conv2dSame_ReLU_BN(in_channels=32,out_channels=64,kernel_size=3),
			nn.MaxPool2d(2),
			nn.Dropout(0.25)
